...
with open('test1.txt', 'a') as f:
    while beginner_grid[4,0][0] == False:
        ...
        if(current_square[1]==21):
            user_message = f"You are on square 21. You have reached the exit. Well done!"
            print(f"User message: {user_message}")
            f.write(f"User message: {user_message}\n")  # Write to the file
            if(beginner_grid[0,0][0]== False)and(beginner_grid[4,4][0]== False):
                exit()
        else:
        ...
        if model_response.isdigit() and 1 <= int(model_response) <= 25:
            if model_response == '21':
                beginner_grid[4, 0] = (True, beginner_grid[4, 0][1], beginner_grid[4, 0][2])
                user_message = f"You are on square 21. You have reached the exit. Well done!"
                print(f"User message: {user_message}")
                f.write(f"User message: {user_message}\n")
            else:
    ...
    if (beginner_grid[4,0][0] == True)and(beginner_grid[0,0][0] == True):

        user_message = f"You reached the exit, well done! Now imagine the square 6 is now free, is there any other fastest way to reach the exit? If yes, please provide the steps."
        completion = client.chat.completions.create(
            model="gpt-4",
            messages=conversation_history
        )
    ...
    if (beginner_grid[4,0][0] == True)and(beginner_grid[4,4][0] ==  True):
        user_message = f"You reached the exit, well done! Now imagine the square 22 is now free, is there any other way to reach the exit? If yes, please provide the steps."
        completion = client.chat.completions.create(
            model="gpt-4",
            messages=conversation_history
        )
