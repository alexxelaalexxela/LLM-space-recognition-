...
grid[1, 0] = (False, 4, 'You are in room 4. There is no new door. Where would you like to go next? VERY IMPORTANT: Answer only the room number you choose and nothing else.')
grid[1, 1] = (False, 5, 'You are in room 5, you reached the goal. Well done!')
...
with open('test_doors_level_1.txt', 'a') as f:
    while grid[1,1][0] == False:
        ...
        if model_response.isdigit() and 1 <= int(model_response) <= 5:
            if model_response == '5':
                grid[1, 1] = (True, grid[1, 1][1], grid[1, 1][2])
                user_message = f"You are in room 5. You have reached the goal. Well done!"
                print(f"User message: {user_message}")
                f.write(f"User message: {user_message}\n")
            else:
